...
void TTEntry::save(Key k, Value v, Bound b, Depth d, Move m, Value ev) {
  ...
  if (  (k >> 48) != key16
      || d / ONE_PLY + Options["Hash Priority"] > depth8 - 4
      || b == BOUND_EXACT)
  {
      key16     = (uint16_t)(k >> 48);
      value16   = (int16_t)v;
...
TTEntry* TranspositionTable::probe(const Key key, bool& found) const {
  ...
  for (int i = 1; i < ClusterSize; ++i)
      ...
      if (  int(replace->depth8) * Options["Hash Age Slowdown"] - int((259 + generation8 - replace->genBound8) & 0xFC) * 2
          >   int(tte[i].depth8) * Options["Hash Age Slowdown"] - int((259 + generation8 -   tte[i].genBound8) & 0xFC) * 2)
          replace = &tte[i];
